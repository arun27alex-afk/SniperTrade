...
if st.session_state['access_token']:
    ...
    if res.get("s") == "ok" and 'candles' in res and len(res['candles']) > 0:
        ...
        with st.sidebar:
            ...
            expiry_str = st.text_input("Enter Expiry (e.g., 26707)", "26707") 
        ...
        if len(active_trades) > 0:
            ...
            act_col2.metric("📌 Spot Entry Price", f"{current_trade['entry']:.2f}")
            act_col3.metric("🎯 Spot Target", f"{current_trade['target']:.2f}")
